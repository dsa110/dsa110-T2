...
K_DEFAULT = 135.0   # recovered_snr ≈ K_DEFAULT * scale
...
def run():
    ...
    for i in range(niterations):
        ...
        for idx, row in enumerate(params):
            ...
            # 2) map to scale with clipping
            current_scale = snr_to_scale_clipped(effective_snr)
            ...
            print(f"[info] Injecting FRB {frbno} with DM={DM}, SNR={effective_snr:.1f}, into global beam {gbeam} using template {template}")
